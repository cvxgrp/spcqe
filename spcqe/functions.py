...
def regularization_matrix_3(K,l,P1,P2,P3):
    ...
    coeff_i=[0] # Do not penalize 0'th Fourier coefficient corresponding to DC signal. 
    for i in range(1,K+1):
        coeff_i.append(l1*i)
        coeff_i.append(l1*i)
    for i in range(1,K+1):
...
def fit_quantiles(y1, B, D, percentiles):
    ...
    cons=[cp.diff(BT,axis=1)>=0.01] #ensures quantiles are in order and prevents point masses ie minimum distance.
    ...
    right_tail_slope=1*(g[:,-2]-g[:,-3])/(Q_extended[:,-2]-Q_extended[:,-3])
